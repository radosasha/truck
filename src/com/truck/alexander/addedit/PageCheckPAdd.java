...
public class PageCheckPAdd extends Activity implements OnClickListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.pagecheckpointadd);
        
        Log.e("New Page Edit", "ON CREATE");
        
         date = (EditText)findViewById(R.checkpointadd.date);
         city = (EditText)findViewById(R.checkpointadd.city);
         memo = (EditText)findViewById(R.checkpointadd.memo);
        
         btAdd = (Button)findViewById(R.checkpointadd.ok);
         btCancel = (Button)findViewById(R.checkpointadd.cancel);
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.checkpointadd.ok:
			if (checkDataSet()) {
				// data is correct
				DBConnector dbOpenHelper = new DBConnector(PageCheckPAdd.this);
				// open connection
				SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
				// добавить insert
				dbOpenHelper.insertInto(db, "checkp"+PageMainGUI.tripid, createRequest());
				dbOpenHelper.close();
				db.close();
		...
		case R.checkpointadd.cancel:
			setResult(RESULT_CANCELED);
			finish();
			break;

		case R.checkpointadd.date:
	...
	private HashMap<String,String> createResultData() {
		...
		rst.put("city", city.getText().toString());
	...
	private boolean checkDataSet() {
		// for each field , except "MEMO","date"
